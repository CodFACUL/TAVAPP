...
with taberp:
    ...
    cl1.metric('Valor Lucro', round(gs_con['Profit'].sum(), 2), '1')
    cl1.metric('Valor Médio Comprado', round(gs_con['Sales'].mean(), 2), '1')
    cl1.metric('Quantidade Comprada', round(gs_con['Quantity'].sum(), 2), '1')
    ...
    st.dataframe(clu_pai[clu_pai['referencia'] == gs_con['Country'].values[0]]
        ['Country', 'm_entrega', 'm_lucro', 'm_vendas', 'm_qtde', 'f_vendas', 'f_lucro', 'r_dias'])
